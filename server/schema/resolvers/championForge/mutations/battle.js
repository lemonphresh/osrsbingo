...
module.exports = {
  ...
  startCFBattle: async (_, { eventId, team1Id, team2Id }, { user }) => {
    ...
    const [team1, team2] = await Promise.all([
      CFTeam.findByPk(team1Id),
      CFTeam.findByPk(team2Id),
    ]);
    ...
    const [items1, items2] = await Promise.all([
      CFItem.findAll({ where: { teamId: team1Id } }),
      CFItem.findAll({ where: { teamId: team2Id } }),
    ]);
    ...
    const battle = await CFBattle.create({
  ...
  submitBattleAction: async (_, { battleId, teamId, action, itemId }, { user }) => {
    ...
    await sequelize.transaction(async (t) => {
      ...
      if (action === 'ATTACK') {
        ...
        const item = await CFItem.findByPk(itemId, { transaction: t });
