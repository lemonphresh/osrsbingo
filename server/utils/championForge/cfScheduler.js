...
/**
 * Shared logic for transitioning a CFEvent to GATHERING.
 * Called by both the GraphQL resolver (manual launch) and the bot cron (scheduled launch).
 */

const { sampleTasksFromPool } = require('./cfTaskSampler');
const { sendCFPhaseAnnouncement } = require('./cfNotifications');
const { pubsub } = require('../../schema/pubsub');

async function triggerGatheringTransition(event) {
  const { CFTeam, CFTask } = require('../../db/models');
  ...
  await CFTask.bulkCreate(taskRows.map((r) => ({ ...r, createdAt: now, updatedAt: now })));
  ...
  pubsub.publish(`CLAN_WARS_EVENT_UPDATED_${event.eventId}`, { cfEventUpdated: event });
  return event;
}
