...
import { Box, VStack, HStack, Text, SimpleGrid, Badge, Button } from '@chakra-ui/react';
    ...
    >
      ...
      >
        <Text
          ...
          mb={1}
          sx={{
            animation: isHit ? 'hitShake 0.4s ease' : undefined,
            '@keyframes hitShake': {
    ...
    for (const h of historicalShots) {
      const isDupe = liveShots.some(
        (l) =>
          l.row === h.row &&
          l.col === h.col &&
          l.firingTeamName === h.firingTeamName &&
          l.shotAt === h.shotAt
      );
    ...
    >
      ...
      <Box maxW="1400px" mx="auto" px={[4, 6, 8]} py={[5, 7]}>
        <VStack align="stretch" spacing={6}>
          {/* Spectator explainer banner — the pulsing "Spectating" label in
              the topbar is easy to miss. This makes it clear the viewer is
              a non-participant seeing both boards for oversight. */}
          <Box
            ...
            py={3}
          >
            <Text fontFamily="mono" fontSize="xs" color="#6b9e78" lineHeight="1.7">
              👁️ You're spectating this campaign. Both team boards are visible so you can follow the
              action. You can't propose or fire shots.
            </Text>
          </Box>
