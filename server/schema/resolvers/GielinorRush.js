...
const logGRActivity = async (eventId, teamId, type, data = {}) => {
  const activity = {
  ...
  try {
    await GRActivity.create(activity);
  } catch (err) {
...
const GielinorRushResolvers = {
  Query: {
    getGREvent: async (_, { eventId }) => {
      logger.info(`[getGREvent] eventId=${eventId}`);
      return GREvent.findByPk(eventId);
    },

    getGRTeam: async (_, { eventId, teamId }) => {
      logger.info(`[getGRTeam] eventId=${eventId} teamId=${teamId}`);
      return GRTeam.findOne({ where: { teamId, eventId } });
    },

    getAllGREvents: async (_, { userId }) => {
      logger.info(`[getAllGREvents] userId=${userId || 'all'}`);
      const where = userId ? { creatorId: userId } : {};
      return GREvent.findAll({ where, order: [['createdAt', 'DESC']] });
    },

    getMyGREvents: async (_, __, context) => {
      if (!context.user) throw new Error('Not authenticated');
      logger.info(`[getMyGREvents] userId=${context.user.id}`);
      return GREvent.findAll({
        where: { creatorId: context.user.id },
        order: [['createdAt', 'DESC']],
      });
    },

    getAssociatedGREvents: async (_, __, context) => {
      if (!context.user) throw new Error('Not authenticated');
      ...
      logger.info(`[getAssociatedGREvents] userId=${userId} discordUserId=${discordUserId}`);

      // Events where user is creator, admin, or ref (any status — caller filters)
      const staffEvents = await GREvent.findAll({
        where: {
      ...
      if (discordUserId) {
        ...
        if (memberEventIds.length > 0) {
          memberEvents = await GREvent.findAll({
            where: {
    ...
    getPendingSubmissions: async (_, { eventId }) => {
      ...
      return GRSubmission.findAll({
        where: { status: 'PENDING_REVIEW' },
        include: [{ model: GRTeam, as: 'team', where: { eventId } }],
        order: [['submittedAt', 'ASC']],
      });
    },

    getGREventLeaderboard: async (_, { eventId }) => {
      logger.info(`[getGREventLeaderboard] eventId=${eventId}`);
      return GRTeam.findAll({ where: { eventId }, order: [['currentPot', 'DESC']] });
    },

    getAllSubmissions: async (_, { eventId }) => {
      const [submissions, nodes] = await Promise.all([
        GRSubmission.findAll({
          where: {
            [Op.or]: [
    ...
    getNodeSubmissionSummaries: async (_, { eventId }) => {
      ...
      const subs = await GRSubmission.findAll({
    ...
    getGRActivities: async (_, { eventId, limit = 50, offset = 0 }) => {
      logger.info(`[getGRActivities] eventId=${eventId} limit=${limit} offset=${offset}`);
      const rows = await GRActivity.findAll({
        where: { eventId },
  ...
  Mutation: {
    createGREvent: async (_, { input }, context) => {
      if (!context.user) throw new Error('Not authenticated');
      logger.info(`[createGREvent] userId=${context.user.id} eventName="${input.eventName}"`);
      ...
      const event = await GREvent.create(eventData);
      logger.info(`[createGREvent] ✅ created eventId=${eventId}`);
      return event;
    },
    ...
    generateGRMap: async (_, { eventId }) => {
      logger.info(`[generateGRMap] eventId=${eventId}`);
      const sequelize = GREvent.sequelize;
      ...
      try {
        ...
        await GRNode.destroy({ where: { eventId }, force: true, transaction });
        ...
        await GRNode.bulkCreate(validatedNodes, { transaction, validate: true });
        ...
        logger.info(
          `[generateGRMap] ✅ map generated for eventId=${eventId} nodes=${validatedNodes.length}`
        );
        return GREvent.findByPk(eventId);
      } catch (error) {
        if (transaction && !transaction.finished) await transaction.rollback();
        logger.error(`[generateGRMap] ❌ failed for eventId=${eventId}:`, error.message);
    ...
    deleteGREvent: async (_, { eventId }) => {
      logger.info(`[deleteGREvent] eventId=${eventId}`);
      const event = await GREvent.findByPk(eventId);
      if (!event) throw new Error('Event not found');
      invalidateEventNodes(eventId);
      await event.destroy();
      logger.info(`[deleteGREvent] ✅ deleted eventId=${eventId}`);
      return { success: true, message: 'Event deleted successfully' };
    },

    deleteGRTeam: async (_, { eventId, teamId }) => {
      logger.info(`[deleteGRTeam] eventId=${eventId} teamId=${teamId}`);
      const team = await GRTeam.findOne({ where: { teamId, eventId } });
      if (!team) throw new Error('Team not found');
      await team.destroy();
      logger.info(`[deleteGRTeam] ✅ deleted teamId=${teamId}`);
      return { success: true, message: 'Team deleted successfully' };
    },

    createGRTeam: async (_, { eventId, input }, context) => {
      logger.info(`[createGRTeam] eventId=${eventId} teamName="${input.teamName}"`);
      const nodes = context.loaders
        ? await context.loaders.nodesByEventId.load(eventId)
        : (
            await GREvent.findByPk(eventId, {
              include: [{ model: GRNode, as: 'nodes' }],
            })
          )?.nodes;
      ...
      const team = await GRTeam.create({
        teamId,
        eventId,
        ...input,
        currentPot: '0',
      ...
      logger.info(`[createGRTeam] ✅ created teamId=${teamId} startNode=${startNodeId}`);
      return team;
    },
    ...
    submitNodeCompletion: async (
      ...
      const [event, team] = await Promise.all([
        ...
        GRTeam.findOne({ where: { teamId, eventId } }),
      ]);
      ...
      const submission = await GRSubmission.create({
        submissionId,
        teamId,
        nodeId,
        submittedBy,
    ...
    reviewSubmission: async (_, { submissionId, approved, reviewerId, denialReason }, context) => {
      ...
      const submission = await GRSubmission.findByPk(submissionId, {
        include: [{ model: GRTeam, as: 'team' }],
      });
    ...
    adminCompleteNode: async (_, { eventId, teamId, nodeId, congratsMessage }, context) => {
      ...
      await logGRActivity(eventId, teamId, 'node_completed', {
        nodeId,
      ...
      if (node.rewards?.gp > 0) {
        await logGRActivity(eventId, teamId, 'gp_gained', {
      ...
      const submissions = await GRSubmission.findAll({ where: { nodeId, teamId } });
      ...
      if (nodes.length > 0) {
        ...
        if (allCompleted) {
          const allTeamSubmissions = await GRSubmission.findAll({
            where: { teamId },
    ...
    adminUncompleteNode: async (_, { eventId, teamId, nodeId }, context) => {
      ...
      await logGRActivity(eventId, teamId, 'node_uncompleted', {
        nodeId,
        nodeTitle: node.title,
    ...
    adminSilentReCompleteNode: async (_, { eventId, teamId, nodeId }, context) => {
      ...
      await logGRActivity(eventId, teamId, 'node_recompleted', {
        nodeId,
        nodeTitle: node.title,
    ...
    applyBuffToNode: async (_, { eventId, teamId, nodeId, buffId }, context) => {
      ...
      const [event, team, node] = await Promise.all([
        GREvent.findByPk(eventId, { attributes: ['eventId', 'startDate'] }),
        GRTeam.findOne({ where: { teamId, eventId } }),
        GRNode.findByPk(nodeId),
      ]);
    ...
    adminRemoveBuffFromNode: async (_, { eventId, teamId, nodeId }, context) => {
      ...
      const [team, node] = await Promise.all([
        GRTeam.findOne({ where: { teamId, eventId } }),
        GRNode.findOne({ where: { nodeId, eventId } }),
      ]);
      if (!team) throw new Error('Team not found');
    ...
    adminRefundInnPurchase: async (_, { eventId, teamId, nodeId }, context) => {
      ...
      const team = await GRTeam.findOne({ where: { eventId, teamId } });
      if (!team) throw new Error('Team not found');
      ...
      await logGRActivity(eventId, teamId, 'inn_refunded', {
        innId: nodeId,
...
module.exports = GielinorRushResolvers;
