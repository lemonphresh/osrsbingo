...
const { buildSEBracket, buildDEBracket } = require('../../../../utils/championForge/cfBracket');
...
module.exports = {
  ...
  lockCFLoadout: async (_, { teamId }, { user }) => {
    if (!user) throw new AuthenticationError('Not authenticated');
    const team = await getTeamOrThrow(teamId);
    const event = await getEventOrThrow(team.eventId);
  ...
  adminLockAllLoadouts: async (_, { eventId }, { user }) => {
    ...
    return CFTeam.findAll({ where: { eventId } });
  },

  generateCFBracket: async (_, { eventId, bracketType }, { user }) => {
    if (!user) throw new AuthenticationError('Not authenticated');
    const event = await getEventOrThrow(eventId);
    if (!isAdmin(event, user.id)) throw new AuthenticationError('Not an event admin');
