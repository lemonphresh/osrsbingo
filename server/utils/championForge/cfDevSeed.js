...
async function seedDevCfEvent(userId) {
  const {
    CFEvent, CFTeam, CFItem,
    CFBattle, CFBattleEvent, CFTask, CFSubmission,
  } = require('../../db/models');
  ...
  for (const item of allItems) await CFItem.create(item);
...
const OTHER_CF_SEEDERS = [
  ...
  '../db/seeders/20260325000009-cw-outfitting-full-seeder',
...
async function seedAllCfEvents(userId, { discordId, discordUsername } = {}) {
  ...
  const { CFEvent, CFTeam } = require('../../db/models');
  const uid = String(userId);

  // Upsert userId into adminIds for every event
  const events = await CFEvent.findAll({ where: { eventId: ALL_CF_EVENT_IDS } });
