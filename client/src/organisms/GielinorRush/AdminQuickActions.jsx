...
import { CHECK_DISCORD_CHANNELS, GET_GR_ACTIVITIES } from '../../graphql/queries';
import {
...
const AdminQuickActionsPanel = ({
  ...
  const [fetchActivities, { loading: activitiesLoading }] = useLazyQuery(GET_GR_ACTIVITIES, {
    fetchPolicy: 'network-only',
  });
  ...
  const handleOpenFullHistory = () => {
    ...
    fetchActivities({ variables: { eventId: event.eventId, limit: PAGE_SIZE, offset: 0 } }).then(
      ({ data }) => {
        const results = data?.getGRActivities ?? [];
    ...
    try {
      ...
      await Promise.all([
        ...
        fetchActivities({
          ...
          setAllActivities(data?.getGRActivities ?? []);
          setLocallyUncompleted(new Set());
        }),
      ]);
    } finally {
