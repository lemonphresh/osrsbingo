...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const { CFEvent, CFTeam, CFTask, CFSubmission } = getModels();

    // Look up the task
    const task = await CFTask.findOne({ where: { taskId, isActive: true } });
    if (!task) {
      return message.reply(`❌ Task \`${taskId}\` not found. Check your task ID and try again.`);
    }

    // Look up the event — must be in GATHERING
    const event = await CFEvent.findByPk(task.eventId);
...
module.exports.cfpresubmit = {
  ...
  async execute(message, args) {
    ...
    const { CFEvent, CFTeam, CFTask, CFPreScreenshot } = getModels();
