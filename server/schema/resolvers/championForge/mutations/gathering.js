...
module.exports = {
  joinTaskInProgress: async (_, { eventId, teamId, taskId }, { user }) => {
    ...
    if (!isEventAdmin) {
      ...
      const task = await CFTask.findByPk(taskId);
  ...
  markTaskComplete: async (_, { eventId, teamId, taskId }, { user }) => {
    ...
    const { CFTeam, CFSubmission, CFItem } = getModels();
    const team = await CFTeam.findByPk(teamId);
    if (!team || team.eventId !== eventId) throw new UserInputError('Team not found');

    const approvedSubs = await CFSubmission.findAll({
    ...
    for (const sub of approvedSubs) {
      ...
      if (!dropResult.success) {
        logger.warn(
          `[CF] markTaskComplete drop failed for sub ${sub.submissionId}: ${dropResult.reason}`
        );
        continue;
      }
      ...
      const createdItem = await CFItem.create({
  ...
  undoTaskComplete: async (_, { eventId, teamId, taskId }, { user }) => {
    ...
    for (const sub of approvedSubs) {
      if (!sub.rewardItemId) continue;
      await CFItem.destroy({ where: { itemId: sub.rewardItemId } });
  ...
  createCFSubmission: async (_, { input }) => {
    ...
    const submission = await CFSubmission.create({
    ...
    await pubsub.publish(`CLAN_WARS_SUBMISSION_ADDED_${input.eventId}`, {
      cfSubmissionAdded: submission,
    });

    return submission;
  },

  reviewCFSubmission: async (
    _,
    ...
    const submission = await CFSubmission.findByPk(submissionId);
  ...
  changeSubmissionRewardSlot: async (_, { submissionId, rewardSlot }, { user }) => {
    if (!user) throw new AuthenticationError('Not authenticated');
    const { CFSubmission, CFItem } = getModels();
    const submission = await CFSubmission.findByPk(submissionId);
    if (!submission) throw new UserInputError('Submission not found');
  ...
  undoSubmissionApproval: async (_, { submissionId }, { user }) => {
    ...
    if (submission.rewardItemId) {
      await CFItem.destroy({ where: { itemId: submission.rewardItemId } });
    }
  ...
  createCFPreScreenshot: async (_, args) => {
    ...
    await pubsub.publish(`CLAN_WARS_PRESCREENSHOT_ADDED_${args.eventId}`, {
      cfPreScreenshotAdded: preScreenshot,
    });
