...
import { GET_GR_EVENT_PUBLIC, GET_GR_EVENT_ADMIN_TEAMS, GET_ALL_SUBMISSIONS, GET_NODE_SUBMISSION_SUMMARIES } from '../graphql/queries';
...
import MultiTeamGRMapVisualization from '../organisms/GielinorRush/MultiTeamGRMapVisualization';
import { useAuth } from '../providers/AuthProvider';
import { FaCog } from 'react-icons/fa';
import DiscordSetupModal from '../molecules/GielinorRush/DiscordSetupModal';
import GameRulesTab from '../organisms/GielinorRush/GRGameRulesTab';
import { useThemeColors } from '../hooks/useThemeColors';
import AdminLaunchChecklist from '../organisms/GielinorRush/AdminChecklist';
import AdminQuickActionsPanel from '../organisms/GielinorRush/AdminQuickActions';
import EventStatusBanner from '../organisms/GielinorRush/EventStatusBanner';
import usePageTitle from '../hooks/usePageTitle';
import LaunchCheckModal from '../organisms/GielinorRush/LaunchCheckModal';
import EventSummaryPanel from '../organisms/GielinorRush/EventSummaryPanel';
import DenialReasonModal from '../organisms/GielinorRush/DenialReasonModal';
import CompleteNodeDialog from '../organisms/GielinorRush/CompleteNodeDialog';

// Extracted components
import DraftGateView from '../organisms/GielinorRush/DraftGateView';
import EventHeroStrip from '../organisms/GielinorRush/EventHeroStrip';
import EventStatBar from '../organisms/GielinorRush/EventStatBar';
import LeaderboardPanel from '../organisms/GielinorRush/LeaderboardPanel';
import SubmissionsTab from '../organisms/GielinorRush/SubmissionsTab';
import EventSettingsTab from '../organisms/GielinorRush/EventSettingsTab';
import AllNodesTab from '../organisms/GielinorRush/AllNodesTab';
import AdminLaunchFAQModal from '../organisms/GielinorRush/AdminLaunchFAQModal';
...
const GREventView = () => {
  ...
  } = useQuery(GET_GR_EVENT_PUBLIC, { variables: { eventId } });

  const publicEvent = publicEventData?.getGREvent;
  ...
  const event = useMemo(() => {
    ...
    if (!adminTeamsData?.getGREvent) return publicEvent;
    const adminTeamMap = new Map(
      adminTeamsData.getGREvent.teams.map((t) => [t.teamId, t])
    );
    return {
      ...publicEvent,
  ...
  // ── Mutations ──────────────────────────────────────────────────────────────
  const [generateMap, { loading: generateLoading }] = useMutation(GENERATE_GR_MAP, {
    refetchQueries: ['GetGREventPublic', 'GetGREventAdminTeams', 'GetAllSubmissions', 'GetNodeSubmissionSummaries'],
  ...
  useSubscription(GR_ACTIVITY_SUB, {
    variables: { eventId },
    onData: ({ data }) => {
      const activity = data?.data?.grActivity;
  ...
  useEffect(() => {
    if (
      ...
      !notificationsEnabled &&
      !localStorage.getItem('grush_notif_prompt_seen')
    ) {
    ...
    >
      ...
      <Modal isOpen={isNotifPromptOpen} onClose={onNotifPromptClose} isCentered size="md">
        ...
        <ModalContent bg={currentColors.cardBg}>
          <ModalCloseButton
            onClick={() => {
              localStorage.setItem('grush_notif_prompt_seen', 'true');
              onNotifPromptClose();
            }}
          />
