 ...
 *
 * Shared by the GraphQL resolver (createCFEvent) and dev seeders so
 * both use identical logic to build the task list for an event.
 *
 * sampleTasksFromPool(eventId, seed, eventDifficulty)
 *   Returns an array of task row objects ready for CFTask.bulkCreate.
 *   Callers should add { createdAt, updatedAt } before persisting.
 */

const seedrandom = require('seedrandom');
const { getCfTaskPool } = require('./contentRegistry');

const CW_OBJECTIVE_COLLECTIONS = getCfTaskPool();
