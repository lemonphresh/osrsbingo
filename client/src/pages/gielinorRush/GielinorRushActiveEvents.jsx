...
import { GET_ALL_GR_EVENTS, GET_ASSOCIATED_GR_EVENTS } from '../../graphql/queries';
...
const GielinorRushActiveEvents = () => {
  ...
  const { data, loading, error } = useQuery(GET_ALL_GR_EVENTS);
  if (error) console.error('[GielinorRushActiveEvents] query error:', error);

  const { data: assocData } = useQuery(GET_ASSOCIATED_GR_EVENTS, {
    skip: !user,
    fetchPolicy: 'cache-and-network',
  });

  const myEvents = useMemo(() => {
    if (!user || !assocData?.getAssociatedGREvents) return [];
    const uid = String(user.id);
    return assocData.getAssociatedGREvents.map((event) => {
...
export default GielinorRushActiveEvents;
